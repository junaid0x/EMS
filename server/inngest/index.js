...
// Send Email to Admin, if admin does not take action on leave application within 24 hrs:
...
// Cron: Check attendance at 11:30 AM and email absent employee
  ...
  { id: "attendance-reminder-cron", triggers: [{cron: '0 0 6 * * *'}]}, 
   //06:00 UTC = 11:30 AM IST
    async ({step}) => {
      ...
      const absentEmployees = activeEmployees.filter((e)=>!onLeaveIds.includes(emp._id) && !checkedInIds.includes(emp._id))
