...
const Employees = () => {
  ...
  return (
    <div className='animate-fade-in'>
        ...
        {loading ? (
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5'>
            {filtered.length === 0 ? (
              ...
              filtered.map((e)=> <EmployeeCard key={e.id} employee={e} onDelete={fetchEmployees} onEdit={(e) => setEditEmployee(e)}/>)
            ) }
          </div>
        )}
            ...
            <div className='fixed bg-black/40 backdrop-blur-sm inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto' onClick={()=>setShowCreateModal(false)}>
                ...
                <div className='relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8 animate-fade-in' onClick={(e)=> e.stopPropagation()}>
                  <div className='flex items-center justify-between p-6 pb-0'>
                    ...
                    <button onClick={()=>setShowCreateModal(false)} className='p-2 rounded-lg hover:bg-slate-100 transition-colors texxt-slate-400 hover:text-slate-600'>
                      <X className='w-5 h-5'/>
                    </button>
                  </div>
                  <div className='p-6'>
                    <EmployeeForm onSuccess={()=>{setShowCreateModal(false); fetchEmployees();}} 
        ...
        {/* {Edit Employee} */}
        {editEmployee && (
          <div onClick={()=>setEditEmployee(null)} className='fixed inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto
          bg-black/40 backdrop-blur-sm'>
            <div onClick={(e)=> e.stopPropagation()} className='relative bg-white rounded-2xl shadow-2xl w-full 
            max-w-3xl my-8 animate-fade-in'>
              <div className='flex items-center justify-between p-6 pb-0'>
                <div>
                    ...
                    <button onClick={()=>setEditEmployee(null)} className='p-2 rounded-lg hover:bg-slate-100 transition-colors texxt-slate-400 hover:text-slate-600'>
                      <X className='w-5 h-5'/>
                    </button>
              </div>
                <div className='p-6'>
