...
import { CalendarIcon, ChevronsRightIcon, DollarSignIcon, FileTextIcon, LayoutGridIcon, Loader2Icon, LogOutIcon, MenuIcon, SettingsIcon, UserIcon, XIcon } from 'lucide-react'
...
const Sidebar = () => {
  ...
  const navItems = [
    ...
    {name: "Attendance", href: "/attendence", icon: CalendarIcon},
  ...
  const sidebarContent = (
    <>
      {/* {Brand Header} */}
        <div className='px-5 pt-6 border-b border-white/6 pb-5'>
          ...
          <div className='mx-3 mt-4 mb-1 p-3 rounded-lg bg-white/3 border border-white/4'>
            <div className='flex items-center gap-3'>
              ...
              <div className='min-w-0'>
                ...
                <p className='text-[11px] text-slate-500 truncate'>{role === "ADMIN"? "Administrator" : "Emloyee"}</p>
              </div>
            </div>
          </div>
      )}
      ...
      <div className='px-5 pt-5 pb-2'>
        <p className='text-[-10] font-semibold uppercase tracking-[0.12em] text-slate-500'>Navigation</p>
      </div>
      ...
      <div className='flex-1 px-3 space-y-0.5 overflow-y-auto'>
        {loading ? (
          ...
          return(
            <Link key={item.name} to={item.href} className={`group flex items-center gap-3 px-3 py-2.5 rouned-md text-[13px] font-medium transition-all duration-150 relative
            ${isActive ? "bg-indigo-500/12 text-indigo-300" : " text-slate-300 hover:text-white hover:bg-white/4"  }`}>
              ...
              <item.icon className={`isActive? "text-indigo-300" : "text-slate-400 group-hover:text-slate-300"`}/>
  ...
  return (
    <>
      ...
      <button onClick={()=>setMobileOpen(true)} className='lg:hidden fixed top-4 left-4 z-50 p-2 bg-slate-900 texxt-white rounded-lg shadow-lg border border-white/10'>
        <MenuIcon size={20}/>
      </button>

      {/* Mobile overlay */}
      {mobileOpen && <div className='lg:hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-440' onClick={()=>setMobileOpen(false)} />}
      ...
      {/* {Sidebar - Mobile} */}
      <aside className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-linear-to-b from-slate-900 via-slate-900 to-slate-950 text-white z-50 flex flexx-col transform transition-transform 
